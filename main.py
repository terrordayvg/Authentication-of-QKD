...
#Main to be iterated
def run_circ(shots,dist,j,c_key,backend,wait,T_photons,user,rho1,rho2):
...
#Main process
def CX_BB84_noisy(cores,rho1,rho2,backend):
	Vec=[]
	Store_V=[]
	V_count=0
	nusers=1
	...
	for shots in range(1,maxshots,2): #shots increase
		for user in range(1,nusers+1,1):
			T_photons=[[0,0]]*shots
			c_key=Gen_key(shots,user)
			args = [[shots,dist,i,c_key[i],backend,wait,T_photons[i],user,rho1,rho2] for i in range(shots)]
			V_count=0
...
if __name__ == "__main__":
	...
	cores=1																   # Cores for multi-processing
	...
	CX_BB84_noisy(cores,rho1,rho2,backend) 								   #Main processing